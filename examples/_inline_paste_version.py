"""openlab-style v0.1.0 — single-file paste version for ChatGPT Code Interpreter.

When `pip install` is blocked or `pip install git+https://...` fails, paste this
entire file as a Python cell. Then call `apply()` and your matplotlib plots
will look like MATLAB.

Source: github.com/nxcodeio/openlab-style (full project) — MIT license
"""
...
def apply(*, grid=False, fontsize=10):
    """Apply MATLAB-style defaults to matplotlib. Idempotent."""
    try:
        mpl.colormaps["parula"]
    except (KeyError, ValueError):
        mpl.colormaps.register(ListedColormap(PARULA_DATA, name="parula"))
    ...
    mpl.rcParams.update({
        ...
        "legend.fancybox": False, "legend.fontsize": fontsize,
        "image.cmap": "parula",
    })

print("openlab-style loaded. Call openlab_apply() to activate.")
